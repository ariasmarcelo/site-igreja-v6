...
import { triggerStylesRefresh } from '@/hooks/usePageStyles';
...
interface EditableField {
  ...
  currentValue: string | Record<string, string>;  // string para text, Record para styles
...
export default function VisualPageEditor({ 
      ...
      selectors.forEach(selector => {
        ...
        elements.forEach((el, index) => {
          ...
          if (field && field.currentValue) {
            // Para textos (não estilos), currentValue deve ser string
            const textValue = typeof field.currentValue === 'string' 
              ? field.currentValue 
              : String(field.currentValue);
            htmlEl.textContent = textValue;
          }
        ...
        if (isSVG && iconName) {
          ...
          if (isContainer) {
            ...
            const fieldValue = field 
              ? (typeof field.currentValue === 'string' ? field.currentValue : String(field.currentValue))
        ...
        const saveEdit = () => {
          ...
          allInputs.forEach((input: Element) => {
            ...
            if (prop && currentValue !== originalValue) {
              const cssProperty = prop.replace(/([A-Z])/g, '-$1').toLowerCase();
              console.log(`✏️ MODIFICADO: ${prop} → "${originalValue}" para "${currentValue}"`);
          ...
          if (modifiedCount > 0) {
            console.log(`📝 Total modificado: ${modifiedCount} propriedades`);
            console.log(`📦 cssChanges:`, cssChanges);
          } else {
            console.log(`ℹ️ Nenhuma propriedade CSS foi modificada`);
          }
            ...
            setFields(prev => {
              ...
              if (isSVG) {
                ...
                if (Object.keys(cssChanges).length > 0) {
                  ...
                  // IMPORTANTE: NÃO fazer JSON.stringify aqui! 
                  // O objeto cssChanges será stringificado depois ao enviar para API
                  const stylesValue = cssChanges; // ← FIX: Salvar objeto, não string
                  
                  console.log(`💅 Adicionando campo de estilo: ${stylesKey}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('🔍 CAMPOS MODIFICADOS:', modifiedFields.map(f => ({
        key: f.key,
        isStyle: f.key.includes('styles'),
        currentValueType: typeof f.currentValue,
      ...
      Object.entries(textEdits).forEach(([key, value]: [string, { newText: string }]) => {
        console.log(`   ${key}:`, {
          newText: value.newText.substring(0, 100),
          type: typeof value,
          hasNewText: 'newText' in value
        });
      });
      console.groupEnd();
      ...
      if (Object.keys(textEdits).length > 0) {
        try {
          ...
          console.log('🌐 URL:', API_ENDPOINTS.saveVisualEdits);
          ...
          if (response.ok) {
            ...
            try {
              ...
              errorDetails = errorJson.message || errorJson.details || errorJson.error || errorText;
            } catch {
              // Se não for JSON, usa o texto direto
            }
            
            throw new Error(`API retornou status ${response.status}: ${errorDetails}`);
          }
        } catch (apiError) {
      ...
      try {
        ...
        if (!validateResponse.ok) {
          throw new Error(`Validação falhou: API retornou status ${validateResponse.status}. Dados podem ter sido salvos, mas a confirmação falhou.`);
        }
